...
try {
  ...
  const cam = () => tab.evalJs(`(() => { const c = window.__warlets.renderer.cam.target; return { x: c.x, z: c.z, yaw: window.__warlets.renderer.cam.yaw }; })()`);
  // spawns are random, so start from the map centre - a base on the right edge leaves no room to scroll right
  await tab.evalJs(`(() => { const v = window.__warlets, m = v.sim.map; v.centerOn(m.w / 2, m.h / 2); })()`);
  await sleep(300);
  const c0 = await cam();
  ...
  const dmg = await tab.evalJs(`(async () => { const v = window.__warlets; const sim = v.sim, w = sim.world; let castle = -1; for (let id = 0; id < w.maxId; id++) if (w.alive[id] && w.kind[id] === 2 && w.owner[id] === v.mySlot && w.type[id] === 0) castle = id;
  ...
  const rings = await tab.evalJs(`(async () => { const v = window.__warlets, w = v.sim.world; let castle = -1, worker = -1; for (let id = 0; id < w.maxId; id++) { if (!w.alive[id] || w.owner[id] !== v.mySlot) continue; if (w.kind[id] === 2 && w.type[id] === 0) castle = id; if (w.kind[id] === 1 && w.type[id] === 0) worker = id; }
  ...
  await tab.evalJs(`(() => { const v = window.__warlets, w = v.sim.world; for (let id = 0; id < w.maxId; id++) if (w.alive[id] && w.kind[id] === 2 && w.owner[id] === v.mySlot && w.type[id] === 0) { v.centerOn(w.x[id] / 65536, w.y[id] / 65536); return id; } return -1; })()`);
  await sleep(400);
